...
const WeaponsCard = () => {
  ...
  return (
    <div className='bg-gradient-to-l from-[#0F1923] to-[#2C3A4D] text-white min-h-screen px-1 py-10 sm:px-6 md:px-6'>
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 sm:grid-cols-3 gap-4 sm:gap-8 md:gap-10 p-3 ' >
        {weaponSkins
        ...
        .map((skin)=>(
          <div key={skin.uuid} className='flex flex-col justify-center rounded-xl p-3 bg-[#1A1E24] hover hover:scale-105 transform transition duration-300'>
            ...
            <h1 className='text-center mt-5 bg-white text-black rounded text-xs sm:text-xl md:text-xl'>{skin.displayName}</h1>
          </div>
        ))}
